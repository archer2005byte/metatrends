...
function MetaTrendsPresentation() {
  ...
  return (
    <main className="site-shell" onClick={handleCanvasClick}>
      ...
      <section className="panel close-panel" id="today"><div className="close-mark">∞</div><p className="section-no">08 / BACK TO TODAY</p><h2>The future will not be<br /><em>delivered by intelligence alone.</em></h2><p className="close-copy">It will be delivered by institutions that can translate intelligence into operating reality.</p><div className="consulting-grid"><span>STRATEGY</span><span>OPERATING MODEL</span><span>GOVERNANCE</span><span>IMPLEMENTATION</span><span>TRUST</span></div><footer><span>There is time for us.</span><span>EY Managers // Meta Trends</span></footer></section><section className={`panel end-panel ${endRevealed ? "end-open" : ""}`} id="end"><div className="end-reveal" data-no-advance>{qrSrc ? <img src={qrSrc} alt="QR code linking to the Meta Trends resources page" /> : <div className="end-qr-placeholder" />}<p className="end-reveal-label">Go deeper</p><a href={`${assetBase}resources`}>{resourcesUrl ? resourcesUrl.replace(/^https?:\/\//, "") : "/resources"}</a></div></section>
    </main>
  );
}
