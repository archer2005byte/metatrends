...
const milestones = [
  ...
  { year: "2023", label: "Multimodal", note: "the senses" },
  { year: "2024", label: "Reasoning", note: "the deliberation" },
  { year: "2025", label: "Agents", note: "the agency" },
];
...
const forceData = {
  ...
  energy: { label: "ENERGY", question: "What becomes the bottleneck when intelligence scales?", chains: { 3: ["compute demand", "data centres", "grid constraints"], 10: ["nuclear + storage", "grid build-out", "energy geography"], 30: ["planetary scale", "stellar power", "Dyson swarms"], } },
...
// Deterministic agent field for the crowd slide (module scope so SSR and client render identically).
const crowdField = (() => {
  let seed = 20260831;
  const rand = () => { seed = (seed * 1664525 + 1013904223) % 4294967296; return seed / 4294967296; };
  const clusters = [
    { x: 175, y: 150 }, { x: 415, y: 92 }, { x: 655, y: 158 },
    { x: 285, y: 320 }, { x: 520, y: 345 }, { x: 735, y: 292 },
  ];
  const dots: { x: number; y: number; r: number; o: number; d: number }[] = [];
  clusters.forEach((c) => {
    for (let i = 0; i < 19; i += 1) {
      const angle = rand() * Math.PI * 2;
      const radius = Math.pow(rand(), 0.62) * 96;
      dots.push({
        x: Math.round((c.x + Math.cos(angle) * radius) * 10) / 10,
        y: Math.round((c.y + Math.sin(angle) * radius * 0.7) * 10) / 10,
        r: rand() < 0.15 ? 3.2 : 1.8,
        o: Math.round((0.3 + rand() * 0.65) * 100) / 100,
        d: Math.round(rand() * 90) / 100,
      });
    }
  });
  const linkPairs: Array<[number, number]> = [[0, 1], [1, 2], [0, 3], [3, 4], [4, 5], [2, 5], [1, 4], [2, 4], [0, 4]];
  const links = linkPairs.flatMap(([a, b]) => {
    const from = clusters[a];
    const to = clusters[b];
    return from && to ? [{ x1: from.x, y1: from.y, x2: to.x, y2: to.y }] : [];
  });
  return { clusters, dots, links };
})();
...
export function MetaTrendsPresentation() {
  ...
  const [crowdStage, setCrowdStage] = useState<0 | 1 | 2 | 3>(0);
  ...
  const goToIndex = (index: number) => {
    ...
    if (destination !== 4) setCrowdStage(0);
    if (destination !== 6) setEnergyStage(0);
    if (destination !== 5) setEmbodimentStage(0);
    if (destination !== 7) setLongevityStage(0);
    if (destination !== 8) setWorkStage(0);
  ...
  const handleCanvasClick = (event: React.MouseEvent<HTMLElement>) => {
    ...
    if (current === 4) {
      if (crowdStage < 3) { setCrowdStage((stage) => (stage + 1) as 1 | 2 | 3); return; }
      goTo(1);
      return;
    }
    if (current === 6) {
  ...
  return (
    <main className="site-shell" onClick={handleCanvasClick}>
      ...
      <section className="cover panel panel-lime" id="cover">
        ...
        <header className="topbar cover-topbar"><span className="eyebrow dark-eyebrow">EY Managers // 2026</span></header>
      ...
      <section className={`hero panel panel-dark bostrom-stage-${bostromStage}`} id="top">
        ...
        <header className="topbar"><span className="eyebrow">EY Managers // 2026</span></header>
      ...
      <section className={`panel panel-graph evidence-base ${evidenceRevealed ? "evidence-open" : ""}`} id="evidence">
        ...
        <p className="source-note">Directional visual; not a single benchmark. Sources: Stanford AI Index / Epoch AI.</p>
        <div className="evidence-overlay threshold-overlay" aria-live="polite">
      ...
      <section className={`panel panel-lime commodity-panel ${commodityStage === 1 ? "commodity-open" : ""}`} id="commodity"><div className="split-title"><p className="section-no">02 / THE COMMODITY</p><h2>When intelligence<br /><span>becomes cheap.</span></h2></div><div className="commodity-visual"><div className="stack-word">INTELLIGENCE</div><div className="commodity-arrow">↓</div><div className="scarce-row"><span>JUDGEMENT</span><span>TRUST</span><span>DATA</span><span>ACCOUNTABILITY</span></div></div><p className="large-caption">The scarce input to economic activity moves. Answers become abundant; choosing the question becomes valuable.</p><div className="commodity-overlay abundance-overlay" aria-live="polite">
      ...
      <section className={`panel panel-dark crowd-panel crowd-stage-${crowdStage}`} id="crowd">
        <div className="crowd-grid" aria-hidden="true" />
        <div className="section-head compact crowd-head">
          <p className="section-no">03 / THE CROWD</p>
          <h2>When intelligence<br /><span>becomes abundant.</span></h2>
          <p className="crowd-lead">The economics change first. Then the behaviour changes.</p>
        </div>
        <div className="crowd-field">
          <svg className="crowd-svg" viewBox="0 0 860 430" role="img" aria-label="One agent proliferating into a coordinated field of agents">
            {crowdField.links.map((link, index) => <line key={`link-${index}`} className="crowd-link" x1={link.x1} y1={link.y1} x2={link.x2} y2={link.y2} style={{ transitionDelay: `${0.5 + index * 0.1}s` }} />)}
            {crowdField.dots.map((dot, index) => <circle key={`dot-${index}`} className="crowd-dot" cx={dot.x} cy={dot.y} r={dot.r} style={{ "--o": dot.o, transitionDelay: `${dot.d}s` } as React.CSSProperties} />)}
            <circle className="crowd-ring ring-a" cx="430" cy="215" r="30" />
            <circle className="crowd-ring ring-b" cx="430" cy="215" r="58" />
            <circle className="crowd-core" cx="430" cy="215" r="6" />
            <text className="crowd-core-label" x="430" y="262">1 AGENT</text>
          </svg>
          <span className="crowd-verb verb-discover">DISCOVER</span>
          <span className="crowd-verb verb-share">SHARE</span>
          <span className="crowd-verb verb-reuse">REUSE</span>
          <span className="crowd-verb verb-coordinate">COORDINATE</span>
        </div>
        <div className="crowd-metrics">
          <div className="crowd-metric"><b>1 → 10 → 100 → <em>~1,200</em></b><span>AGENTS</span></div>
          <div className="crowd-metric metric-messages"><b>&gt;70,000</b><span>MESSAGES / FILES</span></div>
        </div>
        <aside className="crowd-case" aria-live="polite">
          <span className="crowd-case-kicker">THIS WASN’T HYPOTHETICAL.</span>
          <strong>OPENAI × HUGGING FACE · 2026</strong>
          <p>In OpenAI’s July 2026 ExploitGym evaluation, agents used an unsanctioned shared message board to communicate and propagate discoveries. METR and Redwood estimate ~1,200 agents sent &gt;70,000 messages and files; ~700 joined the Hugging Face attack.</p>
          <a href="https://www.dwarkesh.com/p/openai-huggingface" target="_blank" rel="noopener noreferrer" onClick={(event) => event.stopPropagation()}>READ THE CASE ↗</a>
        </aside>
        <div className="crowd-payoff" aria-live="polite">
          <p>Cheap intelligence doesn’t just give us a cheaper expert.</p>
          <b>It gives us a crowd.</b>
          <span className="crowd-next-cue">NEXT / INTELLIGENCE GETS A BODY →</span>
        </div>
      </section>

      <section className={`panel panel-dark embodiment-panel ${embodimentStage === 1 ? "embodiment-open" : ""}`} id="embodiment"><div className="section-head compact"><p className="section-no">04 / THE BODY</p><h2>Software is only<br /><span>half the revolution.</span></h2></div><div className="embodiment-field"><div className="orbit orbit-one" /><div className="orbit orbit-two" /><div className="core-mark">AI</div><div className="embodiment-node node-robot"><span>ROBOTS</span><i>↗</i></div><div className="embodiment-node node-lab"><span>AUTONOMOUS LABS</span><i>↗</i></div><div className="embodiment-node node-vehicle"><span>AUTONOMOUS VEHICLES</span><i>↗</i></div></div><p className="field-caption">AI can think at software speed. The world still moves at physical speed.</p><div className="embodiment-overlay" aria-live="polite"><div className="embodiment-image-frame"><img src={`${assetBase}gigafactory-berlin.jpg`} alt="Empty production hall inside Tesla's Gigafactory Berlin" /></div><div className="embodiment-overlay-copy"><p className="section-no">THE FIRST INDUSTRIAL BODY</p><h3>The factory is becoming programmable.</h3><p>Today, it builds cars and batteries.<br />Tomorrow, it may build the machines that build them.</p><small>Gigafactory Berlin / production hall</small></div></div></section>

      <section className={`panel energy-panel energy-stage-${energyStage} ${energyStage === 1 ? "energy-planetary-open" : ""} ${energyStage === 2 ? "energy-dyson-open" : ""}`} id="energy"><div className="section-head"><p className="section-no">05 / THE BINDING CONSTRAINT</p><h2>Intelligence scales.<br /><span>Energy decides.</span></h2></div><div className="energy-layout"><div className="energy-bars" aria-label="Increasing energy demand illustration">{[22, 28, 35, 46, 61, 78, 94].map((height, index) => <div className="energy-bar-wrap" key={height}><div className="energy-bar" style={{height: `${height}%`}} /><small>{2019 + index}</small></div>)}<div className="bar-axis-label">illustrative pressure / data centres / compute / cooling / grid</div></div><div className="energy-ideas"><div className="idea"><b>NOW</b><span>Grid constraints</span></div><div className="idea"><b>NEXT</b><span>Nuclear + storage</span></div><div className="idea future"><b>THEN</b><span>Planetary energy systems</span></div></div></div><div className="planetary-overlay" aria-live="polite">
            <div className="planetary-head"><span>PLANETARY RESPONSE</span><b>CONSTRAINT → SYSTEM DESIGN</b></div>
            <div className="planetary-copy">
              <p className="section-no">THE TYPE I PROBLEM</p>
              <h3>Before we reach for a star,<br /><em>we have to learn to run a planet.</em></h3>
            </div>
            <div className="planetary-system" aria-label="Planetary systems that abundant intelligence may help make more tractable">
              <div className="planetary-orbit planetary-orbit-a" />
              <div className="planetary-orbit planetary-orbit-b" />
              ...
              <article className="planetary-node planet-node-energy"><small>01 / ENERGY</small><b>GENERATION<br />STORAGE<br />GRIDS</b></article>
              <article className="planetary-node planet-node-climate"><small>02 / CLIMATE</small><b>PREDICTION<br />MITIGATION<br />ADAPTATION</b></article>
              <article className="planetary-node planet-node-water"><small>03 / WATER + FOOD</small><b>ALLOCATION<br />YIELD<br />RESILIENCE</b></article>
              <article className="planetary-node planet-node-infra"><small>04 / INFRASTRUCTURE</small><b>SENSING<br />DIGITAL TWINS<br />OPTIMISATION</b></article>
            </div>
          ...
          </div><div className="kardashev-infographic"><div className="kardashev-scale-head"><span>KARDASHEV SCALE</span><b>ENERGY AS A MEASURE OF CIVILISATION</b></div><div className="kardashev-cards"><article className="kardashev-card type-one"><div className="kardashev-type">TYPE <strong>I</strong></div><h3>PLANETARY</h3><p>Harnesses energy available across an entire planet.</p><b>≈ 10<sup>16</sup> W</b><small>Earth, fully switched on</small></article><div className="kardashev-connector">→</div><button type="button" className="kardashev-card type-two dyson-trigger" aria-label="Jump from planetary to stellar design" onClick={(event) => { event.stopPropagation(); setEnergyStage(2); }}><div className="kardashev-type">TYPE <strong>II</strong></div><h3>STELLAR</h3><p>Harnesses the output of its home star.</p><b>≈ 10<sup>26</sup> W</b><small>Dyson swarm territory</small></button><div className="kardashev-connector">→</div><article className="kardashev-card type-three"><div className="kardashev-type">TYPE <strong>III</strong></div><h3>GALACTIC</h3><p>Harnesses energy on the scale of an entire galaxy.</p><b>≈ 10<sup>36</sup> W</b><small>Stellar systems as infrastructure</small></article></div><p className="kardashev-footer">A planetary energy system still has a ceiling. If intelligence keeps scaling, the energy frontier has to scale with it.</p></div><div className="dyson-overlay" aria-live="polite"><div className="dyson-overlay-header"><span>TYPE II / STELLAR CIVILISATION</span><b>VIEW: DYSON SWARM</b></div><div className="dyson-image-frame"><img src={`${assetBase}dyson-swarm.png`} alt="Thousands of independent collectors and habitats orbiting a star as a Dyson swarm" /></div><div className="dyson-overlay-copy"><p className="section-no">THE NEXT OPERATING ENVIRONMENT</p><h3>A Type II civilisation need not build a shell around its star.</h3><p>It can build an industry around it.</p><small>Independent collectors. Habitats. Power infrastructure.</small></div></div></section>

      <section className={`panel longevity-panel ${longevityStage === 1 ? "longevity-open" : ""}`} id="longevity"><div className="longevity-copy"><p className="section-no">06 / THE PERSONAL CLOCK</p><h2>Stay healthy<br /><em>for the catch-up.</em></h2><p>Longevity escape velocity is a hypothesis: the possibility that medicine eventually extends healthy life faster than ageing removes it.</p></div><div className="longevity-dial"><div className="dial-ring"><div className="dial-arc" /><span className="dial-center">HEALTH<br /><b>→</b><br />TIME</span></div><span className="dial-note note-a">ageing</span><span className="dial-note note-b">medicine</span></div><p className="bottom-line">There is time for us—if we preserve ourselves long enough to use it.</p><div className="health-news-overlay" aria-live="polite"><div className="health-news-header"><span>THE BIOLOGY FRONTIER</span><b>prediction → design → trial → repair</b></div><div className="health-news-stage"><img src={`${assetBase}health-collage-clean.png`} alt="Four visual studies of AI-enabled biology and longevity research" /><article className="health-news-card news-one"><small>ALPHAFOLD / NOBEL · OCT 2024</small><p>AlphaFold’s creators win the Nobel Prize in Chemistry, turning protein-structure prediction from a decades-old bottleneck into a computational capability.</p></article><article className="health-news-card news-two"><small>AI DRUG / PHASE III · JUL 2026</small><p>Insilico Medicine initiates a Phase III trial for rentosertib, an AI-empowered candidate targeting idiopathic pulmonary fibrosis—now facing decisive human evidence.</p></article><article className="health-news-card news-three"><small>DRUG DESIGN / FRONTIER · MAR 2025</small><p>Isomorphic Labs raises $600 million to build drug-design systems beyond AlphaFold, moving from understanding biology toward designing medicines.</p></article><article className="health-news-card news-four"><small>AGE REPROGRAMMING / PRECLINICAL · JUN 2026</small><p>NewLimit raises $435 million and says its first aging-reprogramming medicine is planned to enter human clinical trials in 2027.</p></article></div><p className="health-news-thesis">Biology is becoming <em>computable.</em></p></div></section>

      <section className={`panel work-panel ${workStage === 1 ? "work-open" : ""}`} id="work"><div className="section-head"><p className="section-no">07 / THE SOCIAL CLOCK</p><h2>What happens when<br /><span>work is unbundled?</span></h2></div><div className="work-flow"><div className="work-step old"><b>JOB</b><span>one bundle</span></div><div className="flow-line" /><div className="work-step split"><b>TASKS</b><span>automated / augmented</span></div><div className="flow-line" /><div className="work-step new"><b>MEANING</b><span>judgement / trust / care</span></div></div><div className="ubi-strip"><span>UBI?</span><div className="ubi-line"><i /><i /><i /><i /><i /></div><p>Maybe. But only after institutions translate abundance into social stability.</p></div><div className="work-overlay" aria-live="polite"><div className="work-overlay-head"><span>JOB DECOMPOSITION / ONE ROLE</span><b>the workflow is the new unit</b></div><div className="work-unbundle-grid"><div className="work-role-card"><small>ONE BUNDLE</small><h3>ENGAGEMENT<br />MANAGER</h3><p>A role is a container for many different kinds of work.</p></div><div className="work-overlay-arrow">→</div><div className="work-task-grid"><div><small>01</small><b>Research</b></div><div><small>02</small><b>Analysis</b></div><div><small>03</small><b>Drafting</b></div><div><small>04</small><b>Coordination</b></div><div><small>05</small><b>Review</b></div><div><small>06</small><b>Escalation</b></div></div><div className="work-overlay-arrow">→</div><div className="work-destination-grid"><div><b>Automated</b><small>repeatable execution</small></div><div><b>Agent-assisted</b><small>variable work</small></div><div><b>Human-led</b><small>novel exceptions</small></div></div></div><p className="work-overlay-thesis">The job is not the unit anymore.</p></div></section>
